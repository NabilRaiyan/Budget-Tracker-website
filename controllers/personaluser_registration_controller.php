...
if (isset($_REQUEST['Submit'])){
    ...
    if (!$hasError == 1){
        $mydb = new Model();
        $conn = $mydb->OpenCon();
        $result = $mydb->addUserIntoRegistration($conn, "registration", $business_email, $business_pass, $business_owner_name, "small-business");
        $result2 = $mydb->addUserIntoSmallBusiness($conn, "smallbusinessuser", $business_type, $business_name, $business_bin_num, $business_monthly_income, $business_email, $business_pass, "null");

        if ($result2 === TRUE){
            $registration_success_message = "Successfully Registered. Please login.";
           
        }else{
            $registration_error_message = "Registration unsuccessful. Please try again with proper information." . $conn->error;
        }}
        else{
            $registration_error_message = "Registration unsuccessful. Please try again with proper information." . $conn->error;
        }
